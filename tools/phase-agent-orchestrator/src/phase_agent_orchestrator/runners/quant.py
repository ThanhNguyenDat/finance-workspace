...
from ..io import CLIError, atomic_write_json
...
def _last_message(result: object) -> str:
    return redact_text(str(getattr(result, "result", None) or getattr(result, "final_response", None) or getattr(getattr(result, "error", None), "message", None) or ""))
...
def run(argv: list[str]) -> int:
    ...
    session = create_quant_session(
        {"request": "quant-research", "prompt_file": str(prompt_file), "repository": str(root)},
        db=coordinator,
        run_root=root / ".ops/runtime/phase-agents/quant-runs",
    )
    session_id = session["id"]
    append_event(session_id, phase="PLAN", event_type="session.created", safe_payload={"iteration": session["quant_iteration"], "namespace": session["worktree"]}, db=coordinator)
    try:
        admission = admit_session(session_id, db=coordinator, owner_pid=os.getpid())
    except CoordinatorError as exc:
        raise CLIError(f"{PREFIX}: {exc}") from exc
    if not admission["admitted"]:
        append_event(session_id, phase="PLAN", event_type="session.queued", safe_payload={"reason": admission["reason"]}, db=coordinator)
        print(f"{PREFIX}: session {session_id} queued: {admission['reason']}", file=sys.stderr)
        return 2
    admission_token = admission["fencing_token"]
    append_event(session_id, phase="PLAN", event_type="session.admitted", safe_payload={"slot_id": admission["slot_id"]}, db=coordinator)
    ...
    try:
        ...
        for index, item in enumerate(options, start=1):
            ...
            update_checkpoint(
                session_id,
                {"iteration": iteration, "attempt": index, "provider": provider, "account": account or None, "result_class": result_class, "continuation": continuation, "worktree_fingerprint_before": fingerprint_before, "worktree_fingerprint_after": fingerprint_after},
                expected_version=current_session["version"],
                fencing_token=admission_token,
                db=coordinator,
            )
            atomic_write_json(stdout.with_suffix(".meta.json"), {"iteration": iteration, "attempt": index, "provider": provider, "model": model, "effort": effort, **({"account": account} if account else {}), "continuation": continuation, "result_class": result_class, "worktree_fingerprint_before": fingerprint_before, "worktree_fingerprint_after": fingerprint_after})
            last_status = status
            if status == 0:
                if account:
                ...
                append_event(session_id, phase="PLAN", event_type="session.completed", safe_payload={"iteration": iteration, "provider": provider}, db=coordinator)
                current_session = get_session(session_id, db=coordinator)
                if current_session is None:
                    raise CLIError(f"{PREFIX}: coordinator session disappeared")
                complete_session(session_id, expected_version=current_session["version"], fencing_token=admission_token, db=coordinator)
