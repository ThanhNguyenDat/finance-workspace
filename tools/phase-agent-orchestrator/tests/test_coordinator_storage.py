...
from phase_agent_orchestrator.coordinator import (
    CoordinatorDB,
...
def make_db(tmp_path: Path) -> CoordinatorDB:
    return CoordinatorDB(path=tmp_path / "coordinator" / "coordinator.db", busy_timeout_ms=2_000)
...
def test_concurrent_quant_sessions_get_unique_iterations_and_namespaces(tmp_path: Path):
    ...
    def create(index: int) -> dict:
        ...
        return create_quant_session({"prompt": f"prompt-{index}"}, db=db, run_root=run_root)
    ...
    assert {item["worktree"] for item in sessions} == {str(run_root / item["id"]) for item in sessions}
    resumed = resume_session(sessions[0]["id"], db=CoordinatorDB(path=db_path, busy_timeout_ms=5_000))
...
def test_records_are_atomic_and_enforce_session_scoped_uniqueness(tmp_path: Path):
    ...
    with pytest.raises(CoordinatorError, match="event attempt does not belong"):
        append_event(session_id, phase="PLAN", event_type="tool", safe_payload={"ok": True}, attempt_id="missing", db=db)
    event = append_event(session_id, phase="PLAN", event_type="phase.started", safe_payload={"ok": True}, db=db)
    assert event["sequence"] == 1

    with pytest.raises(CoordinatorError, match="session not found"):
        record_question("missing", question_id="q1", safe_payload={"prompt": "approve?"}, expires_at="2099-01-01T00:00:00Z", db=db)
...
def test_checkpoint_write_rejects_stale_fencing_without_partial_update(tmp_path: Path):
    ...
    admitted = admit_session(session["id"], capacity=1, owner_pid=401, owner_start_time="boot", db=db)
    updated = update_checkpoint(session["id"], {"step": "started"}, expected_version=2, fencing_token=admitted["fencing_token"], db=db)
    assert updated["checkpoint"] == {"step": "started"}
    with pytest.raises(StaleVersionError):
        update_checkpoint(session["id"], {"step": "stale"}, expected_version=2, fencing_token=admitted["fencing_token"], db=db)
...
def test_question_and_event_records_are_session_isolated(tmp_path: Path):
    ...
    event = append_event(second["id"], phase="PLAN", event_type="log", safe_payload={"line": "safe"}, db=db)

    assert question["session_id"] == first["id"]
    assert event["session_id"] == second["id"]
    assert db.read("SELECT COUNT(*) FROM events WHERE session_id = ?", (first["id"],))[0][0] == 0
    assert db.read("SELECT COUNT(*) FROM operator_questions WHERE session_id = ?", (second["id"],))[0][0] == 0
...
def test_resume_preserves_context_manifest_and_attempt_references(tmp_path: Path):
    ...
    session = create_session("phase-agent-lifecycle-flow", context, checkpoint={"approved": True}, db=db)
...
def test_admission_persists_backpressure_and_uses_session_fencing(tmp_path: Path):
    db = make_db(tmp_path)
    sessions = [create_session(f"change-{index}", {"request": str(index)}, db=db) for index in range(3)]
    first = admit_session(sessions[0]["id"], capacity=2, owner_pid=101, owner_start_time="start-a", db=db)
    second = admit_session(sessions[1]["id"], capacity=2, owner_pid=102, owner_start_time="start-b", db=db)
    queued = admit_session(sessions[2]["id"], capacity=2, owner_pid=103, owner_start_time="start-c", db=db)
    ...
    heartbeat = heartbeat_session(sessions[0]["id"], first["fencing_token"], lease_seconds=600, db=db)
    ...
    retried = admit_session(sessions[2]["id"], capacity=2, owner_pid=103, owner_start_time="start-c", db=db)
...
def test_resource_leases_are_unique_fenced_and_fail_closed_after_expiry(tmp_path: Path):
    ...
    lease = acquire_resource(first["id"], "account", "claude/personal-02", owner_pid=201, owner_start_time="boot-a", db=db)

    with pytest.raises(CoordinatorConflictError, match="owned by session"):
        acquire_resource(second["id"], "account", "claude/personal-02", owner_pid=202, owner_start_time="boot-b", db=db)
    assert_resource_lease("account", "claude/personal-02", lease["fencing_token"], db=db)
    with pytest.raises(StaleVersionError):
    ...
    connection = db.connect()
    try:
        connection.execute("UPDATE resource_leases SET lease_expires_at = '2000-01-01T00:00:00Z'")
        connection.commit()
    finally:
        connection.close()
...
def test_mutable_sessions_get_disjoint_git_worktrees(tmp_path: Path):
    ...
    def git(*args: str) -> None:
        subprocess.run(["git", *args], cwd=source, check=True, capture_output=True, timeout=10)
    git("init", "-q", "-b", "main")
    ...
    first = create_session("change-a", {"request": "a"}, db=db)
    second = create_session("change-b", {"request": "b"}, db=db)
    first_worktree = allocate_worktree(first["id"], source, db=db, worktree_path=tmp_path / "worktrees" / "first")
    second_worktree = allocate_worktree(second["id"], source, db=db, worktree_path=tmp_path / "worktrees" / "second")
...
def test_provider_account_scopes_allow_two_accounts_but_not_one_account_twice(tmp_path: Path):
    db = make_db(tmp_path)
    first = create_session("change-a", {"request": "a"}, db=db)
    second = create_session("change-b", {"request": "b"}, db=db)
    third = create_session("change-c", {"request": "c"}, db=db)
    first_lease = acquire_account_scope(first["id"], "claude", "personal-02", owner_pid=301, owner_start_time="boot-a", db=db)
    second_lease = acquire_account_scope(second["id"], "claude", "personal", owner_pid=302, owner_start_time="boot-b", db=db)
    ...
    with pytest.raises(CoordinatorConflictError, match="owned by session"):
        acquire_account_scope(third["id"], "claude", "personal-02", owner_pid=303, owner_start_time="boot-c", db=db)


def test_recovery_reclaims_only_confirmed_dead_owner(tmp_path: Path):
    db = make_db(tmp_path)
    dead = create_session("dead", {"request": "dead"}, db=db)
    lease = acquire_resource(dead["id"], "change", "change:dead", owner_pid=99999999, owner_start_time="missing", db=db)
    connection = db.connect()
    try:
        connection.execute("UPDATE resource_leases SET lease_expires_at = '2000-01-01T00:00:00Z'")
        connection.commit()
    ...
    acquire_resource(live["id"], "change", "change:live", owner_pid=current_pid, owner_start_time=current_identity, db=db)
    connection = db.connect()
    try:
        connection.execute("UPDATE resource_leases SET lease_expires_at = '2000-01-01T00:00:00Z' WHERE resource_key = 'change:live'")
        connection.commit()
    finally:
        connection.close()
...
def test_coordinator_redacts_secret_bearing_context_and_events(tmp_path: Path):
    ...
    event = append_event(session["id"], phase="PLAN", event_type="log", safe_payload={"message": "Authorization: Bearer do-not-persist"}, db=db)
...
def test_operator_question_response_is_session_and_token_scoped(tmp_path: Path):
    ...
    record_question(first["id"], question_id="q1", safe_payload={"kind": "approval"}, expires_at="2099-01-01T00:00:00Z", db=db)

    with pytest.raises(StaleVersionError):
        answer_question(first["id"], "q1", "yes", fencing_token=second_admission["fencing_token"], db=db)
