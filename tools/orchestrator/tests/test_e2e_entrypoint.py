...
def test_e2e_submits_then_resumes_one_change_session(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]):
...
def test_e2e_explicit_session_must_match_change(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]):
    monkeypatch.setenv("OPS_ROOT", str(tmp_path))
