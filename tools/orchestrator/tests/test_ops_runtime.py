...
from orchestrator.state import ops_transaction
...
def test_transition_matrix_preserves_all_allowed_and_rejected_edges() -> None:
    ...
    for source in all_phases:
        for target in all_phases:
            assert (f"{source}:{target}" in ops_transaction.TRANSITIONS) == (
                f"{source}:{target}" in accepted
            )


def prepare_origin_change(
    ...
    (tmp_path / "openspec/changes" / change / "specs/capability/spec.md").write_text(
        "spec"
    )
    (change_dir / "runtime/lock/owner.json").write_text(
        json.dumps({"session_id": session})
    )
    (change_dir / "runtime/state.json").write_text(
        json.dumps(
            {
                "change": change,
                "phase": "PLAN",
...
@pytest.mark.parametrize(
    "artifact", ["research/quant/rounds/../evidence.md", "README.md"]
)
def test_trace_origin_rejects_traversal_and_outside_root(
...
def test_repo_lock_rechecks_owner_after_existence_snapshot(
    ...
    locks = {
        repository: tmp_path / f"lock-{index}"
    ...
    monkeypatch.setattr(
        change_lock, "repo_lock_dir", lambda repository: locks[repository]
    )
    monkeypatch.setattr(
        change_lock, "release_repo_locks", lambda _change, _session: None
    )
    ...
    class SnapshotExecutor:
        ...
        def __exit__(self, _type: object, _value: object, _traceback: object) -> None:
            (locks[repositories[0]] / "owner.json").write_text(
                json.dumps(
                    {
                        "pid": str(os.getpid()),
                        "hostname": change_lock.socket.gethostname(),
                    }
                )
            )

    monkeypatch.setattr(
        change_lock, "ThreadPoolExecutor", lambda **_kwargs: SnapshotExecutor()
    )
    ...
    assert json.loads((locks[repositories[0]] / "owner.json").read_text())[
        "pid"
    ] == str(os.getpid())
