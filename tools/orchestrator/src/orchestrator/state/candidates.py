...
def validate_candidate(
    provider: str, model: str, effort: str, account: str | None = None
) -> None:
...
def account_record(
    state: dict[str, Any], provider: str, account: str
) -> dict[str, Any]:
...
def state_valid(value: Any) -> bool:
    ...
    for role in ROLES:
        ...
        if not isinstance(item, dict) or item.get("mode") not in {"auto", "manual"}:
            return False
        if (
            item.get("pinned_provider") is not None
            and item.get("pinned_provider") not in PROVIDERS
        ):
            return False
        if item.get("pinned_account") is not None:
            if item.get("pinned_provider") not in PROVIDERS or not isinstance(
                item["pinned_account"], str
            ):
                return False
            try:
                normalized, _ = resolve_account_dir(
                    item["pinned_provider"], item["pinned_account"], PREFIX
                )
        ...
        for option in candidates:
            if not isinstance(option, dict) or not valid_provider(
                option.get("provider", "")
            ):
                return False
            ...
            if not is_string(model) or not SAFE_IDENTIFIER.fullmatch(model):
                return False
            if option["provider"] == "codex" and effort not in {
                "none",
                "minimal",
                "low",
                "medium",
                ...
                return False
            if option["provider"] == "claude" and effort not in {
                "low",
                "medium",
                "high",
                "xhigh",
            ...
            if "account" in option:
                ...
                try:
                    if (
                        normalize_account(option["account"], PREFIX)
                        != option["account"]
                    ):
                        return False
                    validate_candidate(
                        option["provider"], model, effort, option["account"]
                    )
                except CLIError:
                    return False
    for provider in PROVIDERS:
        ...
        if (
            ...
            or item.get("mode") not in {"auto", "manual"}
        ...
        if accounts is not None:
            ...
            for account_name, account_item in accounts.items():
                if not isinstance(account_name, str) or not isinstance(
                    account_item, dict
                ):
                    return False
                try:
                    ...
                    try:
                        ...
                        return False
                if normalized != account_name or not isinstance(
                    account_item.get("available"), bool
                ):
                    return False
                for key in ("reason", "observed_at", "next_probe_at"):
                    if account_item.get(key) is not None and not is_string(
                        account_item[key]
                    ):
                        return False
    return isinstance(value.get("legacy_imported"), bool) and (
        value.get("updated_at") is None or is_string(value.get("updated_at"))
    )
    ...
    for role in ROLES:
        ...
        claude_index = next(
            (
                ...
                if item["provider"] == "claude" and "account" not in item
            ),
            None,
        )
        if claude_index is None or any(
            item.get("account") in {"personal", "personal-02"}
            for item in options
...
def resolve(role: str, state: dict[str, Any]) -> None:
    ...
    for option in item["candidates"]:
        ...
        if (
            (item["mode"] != "manual" or item["pinned_provider"] == provider)
            and (pinned_account is None or pinned_account == account)
...
def probe_due(provider: str, account: str | None) -> int:
    ...
    try:
        item = (
            state["providers"][provider]
            if account is None
            else state["providers"][provider]
            .get("accounts", {})
            .get(account, availability_record())
        )
        due = (
            (
                item["mode"] == "auto"
                and not item["available"]
                and item["next_probe_at"] is not None
                and item["next_probe_at"] <= utc_now()
            )
            if account is None
            else (
                not item["available"]
    ...
    try:
        ...
        if command == "reset":
            ...
            if account_arg is None:
                state["providers"][provider].update(
                    mode="manual",
                    available=True,
                    reason=None,
                    observed_at=utc_now(),
                ...
                account_record(state, provider, account_arg).update(
                    available=True,
                    reason=None,
                    observed_at=utc_now(),
                    next_probe_at=None,
            ...
            if optional:
                if optional[0].isdigit():
                    cooldown_text, account_arg = (
                        optional[0],
                        optional[1] if len(optional) == 2 else None,
                    )
                else:
            ...
            target = (
                state["providers"][provider]
                if account_arg is None
                else account_record(state, provider, account_arg)
            )
            if result == "success":
                target.update(
                    available=True, reason=None, observed_at=now, next_probe_at=None
                )
                ...
                target.update(
                    ...
                    reason=result,
                    observed_at=now,
                    next_probe_at=utc_after(int(cooldown_text)),
                )
            elif result == "auth-error":
                target.update(
                    available=False, reason=result, observed_at=now, next_probe_at=None
                )
                if account_arg is None:
                    target["mode"] = "manual"
            elif result == "probe-inconclusive":
                target.update(
                    observed_at=now, next_probe_at=utc_after(int(cooldown_text))
                )
            elif result not in {
                "model-unavailable",
