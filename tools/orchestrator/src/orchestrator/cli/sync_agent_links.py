...
Mirrors `.agents/skills/*` and `.agents/rules/*` into each agent-native
tool's own `skills/`/`rules/` directory as relative symlinks, so shared
Finance knowledge has one source of truth (`.agents/`) instead of being
copy-pasted per tool. `--check` reports drift without changing anything.

`TOOLS` lists only agent-native directories that actually exist in this
workspace today. The original version of this tool (before the coordinator
system it lived in was deleted) also targeted `.kimi-code` and `.opencode`;
neither exists in this workspace anymore, so restoring those targets would
recreate directories for tooling this workspace does not use rather than
