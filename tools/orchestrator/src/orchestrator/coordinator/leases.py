...
def max_sessions(value: int | None = None) -> int:
    configured = (
        ...
        else int(os.environ.get("ORCHESTRATOR_MAX_SESSIONS", DEFAULT_MAX_SESSIONS))
    )
    if (
    ...
    *,
    ...
    with coordinator.transaction() as connection:
        ...
        existing = connection.execute(
            "SELECT * FROM admission_slots WHERE session_id = ?", (session_id,)
        ).fetchone()
        if existing is not None:
            ...
            return {
                ...
                "session": _row(updated),
            }
        occupied = {
            int(row["slot_id"])
            for row in connection.execute(
                "SELECT slot_id FROM admission_slots"
            ).fetchall()
        }
        slot_id = next(
    ...
    with coordinator.transaction() as connection:
        ...
        if existing is not None:
            if (
                ...
                and existing["owner_start_time"] == owner_start_time
            ):
...
def recovery_report(
    ...
    with coordinator.transaction(immediate=False) as connection:
        session = connection.execute(
            "SELECT id, status, lease_owner, lease_expires_at, fencing_token, checkpoint FROM sessions WHERE id = ?",
            (session_id,),
        ).fetchone()
        if session is None:
            raise CoordinatorError(f"session not found: {session_id}")
        ...
        resources = connection.execute(
            "SELECT * FROM resource_leases WHERE session_id = ? ORDER BY resource_type, resource_key",
            (session_id,),
        ).fetchall()
        latest_attempt = connection.execute(
            "SELECT status FROM attempts WHERE session_id = ? ORDER BY started_at DESC, rowid DESC LIMIT 1",
            (session_id,),
        ).fetchone()
    ...
    admission_leases = [
        {
            **dict(item),
            ...
            "owner_pid": int(owner_parts[0])
            if owner_parts and owner_parts[0].isdigit()
            else 0,
    ...
    for item in expired:
        ...
        current = (
            process_start_identity(pid) if isinstance(pid, int) and pid > 0 else None
        )
        if current is None:
            dead.append(item)
...
def recover_session(
    ...
    if report["state"] != "RECOVERABLE":
        raise CoordinatorError(
            f"session recovery is {report['state'].lower()}: {report['reason']}"
        )
    coordinator = db or CoordinatorDB()
    with coordinator.transaction() as connection:
        updated = connection.execute(
