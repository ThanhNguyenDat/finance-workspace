...
3. **2 worker live-action bybit không được scrape.** Git liệt kê 6 target
   ...
   `raw/researcher/round208-*.md`) đang giao dịch mà không có metrics.
...
`raw/handoff_agent.md` có ghi một sự cố `502/503` kéo dài ~54 giây mỗi lần deploy
...
`raw/handoff_agent.md` từng ghi bằng chứng "32/32 rules healthy" — nên rule nhiều
