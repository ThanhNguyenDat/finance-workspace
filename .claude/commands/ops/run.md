...
through `uv run --project tools/phase-agent-orchestrator run-phase-agent`.
...
2. Immediately arm `uv run --project tools/phase-agent-orchestrator wait-for-phase-attempt <change>`
   ...
   `uv run --project tools/phase-agent-orchestrator watch-phase-attempt-log <change>` in a second
...
- Logical agents own PLAN, IMPLEMENT, VERIFY, FIX and FINAL_VERIFY; ordered
  ...
  `uv run --project tools/phase-agent-orchestrator configure-phase-agents`.
...
1. Derive one stable kebab-case `<change>`, acquire the change lock, initialize
   ...
   ```text
   uv run --project tools/phase-agent-orchestrator ops-runtime lock <change> <session-id>
   uv run --project tools/phase-agent-orchestrator ops-runtime init <change> <session-id>
   ```
...
3. Identify and lock every affected runtime repository before change-specific
   ...
   ```text
   uv run --project tools/phase-agent-orchestrator ops-runtime lock-repos <change> <session-id> <repo>...
   ```

4. Run PLAN sequentially for each affected repository:

   ```text
   uv run --project tools/phase-agent-orchestrator run-phase-agent <change> <repository> PLAN
   ```
...
```text
uv run --project tools/phase-agent-orchestrator ops-runtime phase <change> <session-id> IMPLEMENT
uv run --project tools/phase-agent-orchestrator run-phase-agent <change> <repository> IMPLEMENT

uv run --project tools/phase-agent-orchestrator ops-runtime phase <change> <session-id> VERIFY
uv run --project tools/phase-agent-orchestrator run-phase-agent <change> <repository> VERIFY
```
...
```text
uv run --project tools/phase-agent-orchestrator ops-runtime fix <change> <session-id>
.ops/changes/<change>/runtime/verification-findings-round-<round>.md
uv run --project tools/phase-agent-orchestrator run-phase-agent <change> <repository> FIX
```
...
```text
uv run --project tools/phase-agent-orchestrator ops-runtime phase <change> <session-id> FINAL_VERIFY
uv run --project tools/phase-agent-orchestrator run-phase-agent <change> <repository> FINAL_VERIFY
```
...
```text
uv run --project tools/phase-agent-orchestrator ops-runtime phase <change> <session-id> RELEASE
uv run --project tools/phase-agent-orchestrator ops-runtime phase <change> <session-id> DEPLOY_VERIFY
uv run --project tools/phase-agent-orchestrator ops-runtime phase <change> <session-id> ARCHIVE
uv run --project tools/phase-agent-orchestrator ops-runtime complete <change> <session-id>
```
...
```text
uv run --project tools/phase-agent-orchestrator ops-runtime cleanup <change> <session-id> <FAILED|BLOCKED>
```
