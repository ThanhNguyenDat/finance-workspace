...
6. Advance the shared lifecycle through BRAINSTORM before implementation:
   ...
   ```text
   uv run --project tools/orchestrator ops-runtime phase <change> <session-id> BRAINSTORM
   uv run --project tools/orchestrator run-phase-agent <change> <repository> BRAINSTORM
   ```
