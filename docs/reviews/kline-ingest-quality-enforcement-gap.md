...
- **finance-live-action (Rust)** — the service that actually owns the
  ...
  postmortems (`docs/archive/legacy-handoff-agent.md` lines ~410, ~417) — genuinely does
...
- **This continuity check has an explicit bypass**: for `market_type ==
  ...
  `docs/archive/legacy-handoff-agent.md`'s already-documented `market_closed`/
...
- [ ] Production: after deploy, confirm live `kline_ingest_invalid_total`
      ...
      concurrent 32-route audit, `docs/archive/legacy-handoff-agent.md`'s
