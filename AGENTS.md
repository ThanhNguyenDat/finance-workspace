...
1. run `uv run --project tools/orchestrator sync-agent-links`;
...
7. run `uv run --project tools/orchestrator sync-agent-links` after changing shared rules or skills;
8. run `uv run --project tools/orchestrator sync-agent-links --check` and verify synchronization.
