...
- [x] 1.1 Confirm no foreign active OPS lock owns an origin record that
...
- [x] 1.2 Record checksums for `.agents/skills/openspec*`,
...
- [x] 2.1 Move quant rounds, index, studies, audits, samples, and reports into
  the `research/quant/` taxonomy with Git-aware moves; verify every tracked
  source path has exactly one destination.
- [x] 2.2 Move operational explanations into `docs/reviews/`, move legacy
...
- [x] 2.3 Compare pre/post content hashes and tracked file counts, then verify
  no top-level `raw/` directory remains and no compatibility symlink recreates
  that namespace.

## 3. Update authoritative contracts and writers

- [x] 3.1 Update `AGENTS.md`, `README.md`, applicable shared rules, and
...
- [x] 3.2 Update `.claude/commands/quant-research.md` and
...
- [x] 3.3 Update `ops-runtime.sh trace-origin` path and resolved-path allowlists
...
- [x] 3.4 Update Agent Contract fixtures and assertions to accept new roots and
...
- [x] 4.1 Update current specs, active change artifacts, documentation, and
  internal research links to their relocated paths; verify all actionable
  repository-relative links resolve.
- [x] 4.2 After confirming the owning OPS lock is released, migrate any existing
...
- [x] 4.3 Update archived narrative references where needed for current-checkout
...
- [x] 5.1 Verify searches find no active `raw/` writer, approved-path contract,
...
- [x] 5.2 Run all bounded `.agents/scripts/tests/*.sh`, shell syntax checks for
...
- [x] 5.4 Compare platform-native checksums from task 1.2 and review the final
