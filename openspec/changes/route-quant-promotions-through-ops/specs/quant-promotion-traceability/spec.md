...
- **THEN** OPS stores one immutable metadata record referencing its research note and metric history without duplicating either file

#### Scenario: Invalid or repeated metadata is rejected
- **WHEN** metadata has a non-positive iteration, unsafe instrument, missing/out-of-scope artifact, wrong session, non-PLAN phase, or an existing origin record
...
- **THEN** its stable change name connects code, delivery evidence, OPS execution evidence, OpenSpec decisions/tasks, research artifacts, and quant metrics
...
- **THEN** they remain audit context marked non-authoritative and only independently qualified active engineering work is promoted through the normal gate
