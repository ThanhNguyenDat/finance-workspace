---
name: quant-research-domain
description: Domain rules for one Finance Live Action BTC/XAU quant-research round — task scope, Alpha/Portfolio search fallback, backtest constraints, result classification, and the promotion gate. Read this for WHAT a round must satisfy; read quant-research-loop for the Claude/Codex round lifecycle (the flow).
---

# Quant Research Domain Rules

Bằng tiếng Việt. Đây là nội dung nghiên cứu/promote của một vòng
quant-research — áp dụng cho bất kỳ ai đang thực thi giai đoạn PLAN hoặc
IMPLEMENT của vòng đó (Claude hoặc Codex), bất kể được gọi qua
`/quant:research`, `quant-research-exec`, hay trực tiếp. File này định nghĩa
**cái gì** một vòng phải thoả mãn; xem
[quant-research-loop](../quant-research-loop/SKILL.md) cho **luồng** điều
phối Claude PLAN/VERIFY ↔ Codex IMPLEMENT/FIX bên ngoài nội dung này.
...
lại cho round sau nếu không chắc). Web search / khảo sát nguồn này là việc
của giai đoạn PLAN (Claude), không giao cho Codex.
...
Sau research/backtest, research truth phải nhất quán:
...
## Khi kết quả là PROMOTE

Chỉ dừng ở việc tạo OpenSpec planning artifact — không có cơ chế tự động
implement/verify nào để giao việc tiếp. Tạo OpenSpec planning artifact
(`/opsx:propose`) luôn là việc của **Claude PLAN**, không bao giờ giao cho
Codex, kể cả khi Codex vừa là bên viết round file:

1. Derive một stable meaningful kebab-case `<change>`; không dùng tên kiểu
   `task-87`, `fix-stuff`, hoặc `research-test`.
2. Proposal/design/tasks/specs phải reference research round, instrument,
   research note và metrics CSV bằng path; không copy toàn bộ research
   report.
3. Dừng lại sau khi OpenSpec change sẵn sàng. Không tự implement, không tự
   gọi lifecycle nào khác.

Nếu classification là `PROMOTE` nhưng giai đoạn hiện tại là Codex IMPLEMENT
(chạy qua `quant-research-exec`), Codex chỉ ghi nhận classification +
evidence trong round file rồi dừng — không tự chạy `/opsx:propose` (Codex
không có quyền truy cập slash-command/skill discovery của Claude Code, và
đây vốn là việc của Claude). Bàn giao lại cho giai đoạn Claude PLAN kế tiếp
trong flow.
...
stable change name và đường dẫn OpenSpec change đã tạo (khi giai đoạn Claude
PLAN đã hoàn tất bước đó). Không hỏi user trong research bình thường và
