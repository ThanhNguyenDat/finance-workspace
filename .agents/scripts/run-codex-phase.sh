...
for command in codex jq timeout git sha256sum; do command -v "$command" >/dev/null 2>&1 || die "$command is required"; done
...
if ! jq -e '.routing_policy_version==1' "$state_file" >/dev/null 2>&1; then
  ...
  [[ "$backend" = codex && ( "$phase" = IMPLEMENT || "$phase" = FIX ) ]] || die 'legacy runtime does not select Codex for this phase'
fi
...
timeout_seconds="${CODEX_TIMEOUT_SECONDS:-3600}"; [[ "$timeout_seconds" =~ ^[1-9][0-9]*$ ]] || die 'invalid timeout'
attempt_id="${PHASE_AGENT_ATTEMPT_ID:-direct-$(date -u '+%Y%m%dT%H%M%S%NZ')-$$}"
[[ "$attempt_id" =~ ^[A-Za-z0-9._:-]+$ ]] || die 'unsafe attempt id'
continuation="${PHASE_AGENT_CONTINUATION:-false}"; case "$continuation" in true|false) ;; *) die 'invalid continuation mode' ;; esac

findings=''
if [[ "$phase" = FIX ]]; then
  findings_file="$RUNTIME_ROOT/.ops/changes/$change/runtime/verification-findings-round-$round.md"
  [[ -s "$findings_file" ]] || die "FIX findings missing: $findings_file"
...
if [[ "$phase" = FINAL_VERIFY ]]; then
  prompt+=$'\nYou are the currently running FINAL_VERIFY attempt. The resolver appends this attempt and derives verification evidence only after your process exits, so do not fail solely because your own record is not yet present in runtime state. Evaluate the committed code, findings, and applicable objective checks. End the final response with exactly these machine-readable lines, using PASS only after all applicable objective checks pass and no P0/P1 remains:\nFINAL_VERIFY_GATE: PASS|FAIL\nP0_FINDINGS: <count>\nP1_FINDINGS: <count>\nOBJECTIVE_GATES: PASS|FAIL'
fi
...
base="${PHASE_AGENT_EVIDENCE_BASE:-$log_dir/codex-${phase,,}-round-$round-$attempt_id}"
...
: >"$last"
set +e
timeout --signal=TERM --kill-after=30s "${timeout_seconds}s" codex exec --ignore-user-config --model "$model" \
  --config "model_reasoning_effort=\"$effort\"" --cd "$workspace_root" "${repository_args[@]}" \
  --ephemeral --dangerously-bypass-approvals-and-sandbox --json --output-last-message "$last" - \
...
if [[ "$phase" = VERIFY || "$phase" = FINAL_VERIFY ]]; then
  if [[ "$workspace_before" != "$(fingerprint "$workspace_root")" || "$repo_before" != "$(fingerprint "$repository_root")" ]]; then
