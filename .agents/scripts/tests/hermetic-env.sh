...
while IFS= read -r variable; do
  case "$variable" in
    PHASE_AGENT_*|AGENT_ROLE_*|OPS_*|QUANT_RESEARCH_*|CODEX_TIMEOUT_SECONDS|CLAUDE_TIMEOUT_SECONDS|CODEX_RESULT_CLASSIFIER|CLAUDE_RESULT_CLASSIFIER|FAKE_*|TRACE)
      unset "$variable"
      ;;
  esac
