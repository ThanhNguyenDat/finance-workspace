...
quant="$ROOT_DIR/.claude/commands/quant-research.md"; ops="$ROOT_DIR/.claude/commands/ops/e2e.md"
...
grep -Fq 'agent-role-state state' "$quant" || fail 'phase state read missing'
grep -Fq 'research_enabled=false' "$quant" || fail 'research gate missing'
for token in XAU BTC REJECTED NO-CHANGE DATA-ISSUE NEEDS-MORE-RESEARCH PROMOTE defensible trace-origin '@.claude/commands/ops/e2e.md'; do grep -Fq "$token" "$quant" || fail "quant contract missing $token"; done
...
grep -Fq 'configure-agent-roles' "$ROOT_DIR/.claude/commands/quant/agent-config.md" || fail 'phase config command missing'
...
git -C "$ROOT_DIR" check-ignore -q -- .ops/runtime/agent-roles/state.json || fail 'phase state not ignored'
