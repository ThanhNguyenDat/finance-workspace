...
grep -Fq './.agents/scripts/run-phase-agent-command.sh quant-research' "$quant" || fail 'manual terminal entrypoint missing'
grep -Fq 'Không gọi' "$quant" && grep -Fq '`begin-iteration`' "$quant" || fail 'double-iteration guard missing'
grep -Fq 'phase-agent-state.sh state' "$quant" || fail 'phase state read missing'
...
grep -Fq 'configure-phase-agents.sh' "$ROOT_DIR/.claude/commands/quant/agent-config.md" || fail 'phase config command missing'
grep -Fq 'run-phase-agent.sh <change> <repository> IMPLEMENT' "$ops" || fail 'OPS implement resolver missing'
