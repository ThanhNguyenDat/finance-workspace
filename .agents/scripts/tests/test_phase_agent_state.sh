...
CONFIG="$ROOT_DIR/tools/orchestrator/bin/configure-phase-agents.sh"
tmp="$(mktemp -d)"
trap 'rm -rf -- "$tmp"' EXIT
...
jq -e '.legacy_imported and (.phases|length==6) and .phases.plan' "$PHASE_AGENT_STATE_DIR/state.json" >/dev/null || fail 'reset-all failed'
