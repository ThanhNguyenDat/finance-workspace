...
RUNNER="$ROOT_DIR/.agents/scripts/run-phase-agent-command.py"; STATE="$ROOT_DIR/.agents/scripts/phase-agent-state.py"
...
set +e; (cd "$workspace" && PHASE_AGENT_QUANT_RESEARCH_PROVIDER=claude PHASE_AGENT_QUANT_RESEARCH_MODEL=opus PHASE_AGENT_QUANT_RESEARCH_EFFORT=low ./.agents/scripts/run-phase-agent-command.py quant-research) >/dev/null 2>&1; invalid_status=$?; set -e
...
set +e
(cd "$workspace" && FAKE_CLAUDE_MODE=quota-delay PHASE_AGENT_QUANT_TIMEOUT_SECONDS=10 ./.agents/scripts/run-phase-agent-command.py quant-research) >/dev/null 2>&1 & first_pid=$!
set -e
...
set +e; (cd "$workspace" && ./.agents/scripts/run-phase-agent-command.py quant-research) >/dev/null 2>&1; concurrent_status=$?; wait "$first_pid"; first_status=$?; set -e
...
set +e; (cd "$workspace" && FAKE_CLAUDE_MODE=delay FAKE_SDK_DELAY_SECONDS=2 PHASE_AGENT_QUANT_TIMEOUT_SECONDS=1 ./.agents/scripts/run-phase-agent-command.py quant-research) >/dev/null 2>&1; status=$?; set -e
