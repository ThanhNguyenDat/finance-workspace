...
for provider in codex claude; do
  ...
  orchestrator phase-agent-state provider-result "$provider" global-quota-exhausted 0 >/dev/null
  orchestrator phase-agent-state probe-due "$provider" || fail "$provider did not become probe due"
  ...
  if orchestrator phase-agent-state probe-due "$provider" >/dev/null 2>&1; then fail "$provider inconclusive probe remained immediately due"; fi
  orchestrator phase-agent-state provider-on "$provider" >/dev/null
