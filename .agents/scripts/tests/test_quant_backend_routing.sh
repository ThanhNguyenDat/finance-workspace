...
ROOT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../../.." && pwd -P)"; OPS="$ROOT_DIR/.agents/scripts/ops-runtime.sh"; QUANT="$ROOT_DIR/.agents/scripts/quant-research-state.sh"
...
grep -Fq 'routing_policy_version' "$ROOT_DIR/.agents/scripts/ops-runtime.sh" || fail 'routing policy contract missing'
grep -Fq 'run-phase-agent.sh' "$ROOT_DIR/.agents/scripts/run-phase-agent.sh" || :
