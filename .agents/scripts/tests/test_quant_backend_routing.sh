...
ROOT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../../.." && pwd -P)"; OPS="$ROOT_DIR/.agents/scripts/ops-runtime.py"; QUANT="$ROOT_DIR/.agents/scripts/quant-research-state.py"
...
grep -Fq 'phase_agent_orchestrator.run_phase_agent' "$ROOT_DIR/.agents/scripts/run-phase-agent.py" || :
