...
if UV_BIN="$(command -v uv 2>/dev/null)"; then
  exec "$UV_BIN" run --project "$PROJECT_DIR" python -m phase_agent_orchestrator.detect_codex_availability "$@"
fi
exec "$PROJECT_DIR/.venv/bin/python" -m phase_agent_orchestrator.detect_codex_availability "$@"
