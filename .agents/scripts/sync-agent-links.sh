...
if UV_BIN="$(command -v uv 2>/dev/null)"; then exec "$UV_BIN" run --project "$PROJECT_DIR" python -m phase_agent_orchestrator.sync_agent_links "$@"; fi
if [[ -x "$PROJECT_DIR/.venv/bin/python" ]]; then exec "$PROJECT_DIR/.venv/bin/python" -m phase_agent_orchestrator.sync_agent_links "$@"; fi
